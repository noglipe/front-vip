...
interface Props {
  ...
  setListaFiltrada?: (lista: TransacoesAPI[]) => void;
}

export default function TabelaT({ transacoes, setListaFiltrada }: Props) {
  ...
  function atualizarTransacaoConcluida(id: number, novoStatus: boolean) {
    const novaLista = dados.filter((transacao) => {
      if (transacao.id === id) {
        return transacao.transacao_concluido === novoStatus;
      }
      return true;
    });

    setTDados(novaLista);
    if (setListaFiltrada) setListaFiltrada(novaLista);
  }
